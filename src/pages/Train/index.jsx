...
import { fetchCurrentMarketPrice, fetchCypherData, finalizePrediction, makePrediction } from '../../utils/TrainingFunctions';
...
const Train = () => {
    ...
    function getNextLevelXP() {
        if (NFTObjects[currentIndex]) {
            let difference = ((Number(NFTObjects[currentIndex][0])+1)*100) - Number(NFTObjects[currentIndex][3]);
            return difference;
        }
    }
    ...
    function getXPPercentage() {
        if (NFTObjects[currentIndex]) {
            let percentage = Number(NFTObjects[currentIndex][3]) / ((Number(NFTObjects[currentIndex][0])+1)*100);
                                ...
                                <div className='x2'>
                                    <div
                                        ...
                                        <div
                                            ...
                                            <select onChange={e => updateCurrentIndex(e.target.value)}>
                                                {NFTObjects[0] ?
                                                    getIDOptions()
                                                    :
                                        ...
                                        <div
                                            ...
                                            {NFTObjects[currentIndex] ?
                                                <div>
                                                    <div className='level'>Level {NFTObjects[currentIndex][0]}</div>
                                                    ...
                                                    <div className='details'>
                                                        <div className='item'>
                                                            SP.ATK: <span>{NFTObjects[currentIndex][4][3]}</span>
                                                        </div>
                                                        <div className='item'>
                                                            SP.DEF: <span>{NFTObjects[currentIndex][4][4]}</span>
                                                        </div>
                                                    </div>
                                                </div>
                                                :
                                        ...
                                        <div
                                            ...
                                            {currentMarketPrice>0 ?
                                                <div className='value'>${String(currentMarketPrice).substring(0,5)}</div>
                                                :
                                                <div className='value'>$-----</div>
                                            }
                                            {currentNFTStats[2] && currentNFTStats[0][0] == currentNFTStats[0][1] ?
                                                <p>Will it go up or down in 10 minutes?</p>
                                                :
                                                <div>
                                                    <p>You predicted {currentNFTStats[3]?"Up":"Down"} from ${getStartPrice()}</p>
                                                    <p>Wait {calculateTimeRemaining()} seconds to finalize</p>
                                                </div>
                                            }
                                        </div>
                                        {currentNFTStats[0] && currentNFTStats[0][0] == currentNFTStats[0][1] ?
                                            <div
                                                className='buttons_wrap'
                                                data-aos='fade-up'
                                                data-aos-offset='0'
                                                data-aos-duration='700'>
                                                    <div onClick={() => predict(NFTObjects[currentIndex][6], true)} className='button contrast'>Up</div>
                                                    <div onClick={() => predict(NFTObjects[currentIndex][6], false)} className='button'>Down</div>
                                            </div>
                                            :
                                            <div
                                                className='buttons_wrap'
                                                ...
                                                data-aos-offset='0'
                                                data-aos-duration='700'>
                                                    <div onClick={() => finalize(NFTObjects[currentIndex][6])} className='button contrast'>Finalize</div>
                                            </div>
                                        }
                                    </div>
                                </div>
